...
## 2026-07-22 additions (staged for the next HTML scene pass — throughput + method)

| Claim | Source |
|---|---|
| SEG_POP (build 7): a third readback mode moves the per-segment popcount INTO the FPGA — 8 KB → 2048 B/row, host popcount deleted; silicon 3 pattern cases 2048/2048 exact, build-6 suite 9/9 non-regression, WNS +0.118; PIM_SEGPOP=1 token-identical on both model families | docs/ROADB_2026_07.md §7; rtl/README.md (build 7); rtl/BUILD7_VERIFICATION.md |
| The honest twist: SEG_POP's byte cut was WALL-NEUTRAL — req-prof showed recv is XDMA-latency-bound (~150–200 µs fixed/read), not byte-bound; the wall is round-trip COUNT, not bytes | docs/ROADB_2026_07.md §7 (req-prof decomposition) |
| Request batching (V2GS) + cross-round program packing (PIM_V2_PACK) took the measured wall 267.1 → 233.0 s/8tok = −12.7%, token-identical, both defaulted on | docs/ROADMAP.md §B.3b/3c; [code:app/test_bitnet_server.cpp] (MAGIC_V2GS, PIM_V2_PACK) |
| The method, made explicit: MVDRAM is a transferable 6-step design method (profile → substrate cost-asymmetry → one organizing constraint → spend layout/encoding/partition → co-design → bound the regime); audited against our system it shows OUR constraint is the host↔DRAM round-trip (we're host-orchestrated over PCIe), not column-to-column movement (theirs, controller-native) | docs/METHOD_MVDRAM_LENS.md |
| Controller-native is reachable on our card: MVDRAM's testbed was DRAM Bender on Alveo U200 — same soft-MC class; "controller-native" = their §V-E execution regime (command bus never waits for the host), a ladder of pipelined issue → streaming ping-pong fetch → on-fabric orchestrator | docs/CONTROLLER_NATIVE.md; docs/STREAMING_FETCH_DESIGN.md |
| Cross-bit-plane accumulator (build 8, ACCUM_XBP): folds the host's place-value sum Σ 2^i·pc into the fabric — one drained vector per group instead of n_bitplanes readbacks; Verilator 128/128 lane-exact incl the negative top plane, failure-set-diff clean vs build 7 | docs/ACCUM_XBP_DESIGN.md; rtl/BUILD8_VERIFICATION.md |
| Calibrate once, run the die: bank 0's tuple transferred verbatim to ALL 16 banks gives byte-identical deposit structure (only per-bank margins differ); full-model inference token-identical on never-calibrated banks 4–7 and on 8 banks at once — 16-bank scale-out is a config exercise | docs/BANK_AUDIT_2026_07.md; docs/CALIBRATION_TRANSFER.md |
| The lattice's long-range coupling is predecoder-block-relative (magnitude shrinks to a ~512-row midpoint, vanishes, flips sign); short-range is position-invariant — the mechanism behind bank-transfer-free vs subarray-transfer-partial | docs/BANK_AUDIT_2026_07.md (boundary atlas) |

# KV ALLOCATOR — staged silicon A/B (Task #70 step 1, 2026-08-04)
...
**Bender→DIMM** (memory `dimm_population_2026_07`): bender 2 = D2 SK hynix
(production compute, dual-rank); bender 0 = D0 hynix; **bender 1 = D1 Micron,
bender 3 = D3 Micron = the storage tier and the real KV home** (MAJ3-dead, valid
RowClone store). Calib files present: `calib_dimm{0,1,2,3}.txt`.
...
| **1c** placement | the fork's collateral coset lands **only on the predicted offset rows** — validates the safe-load offset menu (hynix) and **screens the clean k=1 offset on Micron** | KV-3; unknown #3 |
| **2a** retention | the **Micron (D1/D3) idle-age drift onset** — the number the scrub period is set from | KV-4; unknown #4 |
...
Run the fork probe on the **KV home (Micron D1)** and a **hynix control (D2)**.
The written pattern is the "recompute" reference; `dst_match=8192` == byte-exact
...
# KV home — Micron D1 (bender 1), bank 0, 50 fork iterations:
./rowclone-preserve-exe 1 "$BN/calib_dimm1.txt" 0 50  > /home/deni/Claude/kv_alloc_2026_08_04/ab1a_d1_micron.csv
# KV home — Micron D3 (bender 3):
./rowclone-preserve-exe 3 "$BN/calib_dimm3.txt" 0 50  > /home/deni/Claude/kv_alloc_2026_08_04/ab1a_d3_micron.csv
# hynix control — D2 (bender 2):
./rowclone-preserve-exe 2 "$BN/calib_dimm2.txt" 0 50  > /home/deni/Claude/kv_alloc_2026_08_04/ab1a_d2_hynix.csv
```
**PASS (pre-registered):** every iter `dst_match=8192` (fork byte-exact) AND
`src_match=8192` (prefix source intact) on **all three dies**. Any `src_match<8192`
would mean RowClone consumes/perturbs the shared source → prefix-share by reference
is unsafe and forks must copy-and-keep-golden instead (records the finding, does
not kill the design). RowClone (30,1) is PERFECT on hynix and t23=1..3 on Micron
(MM §2.1) → expect PASS; the Micron runs are the ones that retire unknown #3's
fork half.
...
Anchor to a Micron KV window `[W, W+640)` (use the calib's subarray base as W).
```
# intra-segment: src and tgt both inside one 640-row segment (offset < 640):
PIM_T12=30 ./rowclone-smoke-exe 1 0 <W+16> <W+272> 0xC0FFEE    # expect PERFECT_CLONE 8192/8192
# cross-segment: tgt in the NEXT segment (offset ≥ 640):
PIM_T12=30 ./rowclone-smoke-exe 1 0 <W+16> <W+656> 0xC0FFEE    # expect NOT perfect (intra-subarray law)
```
...
CoW twin must be co-segment (KV-1). (`<W>` filled from `calib_dimm1.txt`'s base.)

### 1c — safe-load coset placement (which rows the fork collaterally writes)
The fork RowClone `(30,1)` sits in the ≤3-NOP co-activation window (C64), so on
hynix it also deposits into the coset of `local(src)⊕local(dst)`. Map the coset so
the allocator reserves those rows dead (safe-load) and never places a live KV page
in them. On Micron, screen which single-generator (k=1) offset is clean.
```
# hynix D2: reproduce the selection-law coset (expect deposits ONLY on lawful members):
PIM_BANK=0 ./spread-test-exe 2 > /home/deni/Claude/kv_alloc_2026_08_04/ab1c_d2_coset.txt
# Micron D1/D3: screen the clean k=1 fork offset (expect ONLY the ⊕generator partner dirty):
PIM_BANK=0 PIM_CALIB_LINE="$(head -1 $BN/calib_dimm1.txt)" ./spread-test-exe 1 \
    > /home/deni/Claude/kv_alloc_2026_08_04/ab1c_d1_coset.txt
PIM_BANK=0 PIM_CALIB_LINE="$(head -1 $BN/calib_dimm3.txt)" ./spread-test-exe 3 \
    > /home/deni/Claude/kv_alloc_2026_08_04/ab1c_d3_coset.txt
```
**PASS:** hynix reproduces the lawful coset (deposits on predicted members only —
MM §2.3/§2.4); Micron shows a **single clean k=1 offset** whose only collateral is
the `⊕generator` partner (that partner becomes a reserved dead slot in the KV page
layout). Output feeds the allocator's per-die safe-offset table (`pim_kv_alloc.py`).
...
### 2a — idle-age drift on the Micron KV home (measures the onset)
```
# write a known pattern to a KV row on D1, then read back at increasing idle ages:
python3 kv_retention_probe.py --bender 1 --calib calib_dimm1.txt --bank 0 \
    --mode idle --ages 0.1,10,30,60,120,300 \
    > /home/deni/Claude/kv_alloc_2026_08_04/ab2a_d1_retention.csv
# repeat --bender 3 (D3).  hynix D2 as the cross-die reference:
python3 kv_retention_probe.py --bender 2 --calib calib_dimm2.txt --bank 0 \
    --mode idle --ages 0.1,10,30,60,120,300 \
    > /home/deni/Claude/kv_alloc_2026_08_04/ab2a_d2_retention.csv
```
**Pre-registered expectation:** mismatched-cells CDF rising with age; hynix onset
≈30 s (b2)/120 s (b0) per the drift ladder (MM §2.5). **Micron onset is UNKNOWN —
this arm sets it.** The scrub period (KV-4) = ½ · min(measured onset across D1/D3).

### 2b — scrubbed: read-rewrite at period < onset → 0 mismatch
```
python3 kv_retention_probe.py --bender 1 --calib calib_dimm1.txt --bank 0 \
    --mode scrub --scrub-period 15 --total 300 \
    > /home/deni/Claude/kv_alloc_2026_08_04/ab2b_d1_scrub.csv
```
...
python3 kv_retention_probe.py --bender 1 --calib calib_dimm1.txt --bank 0 \
    --mode read-only --read-period 5 --total 300 \
    > /home/deni/Claude/kv_alloc_2026_08_04/ab2c_d1_readrefresh.csv
```
...
|---|---|---|
| 1a | dst 8192/8192 AND src 8192/8192, all 3 dies | src-perturbed ⇒ prefix-share needs golden-keep, not by-reference (record) |
| 1b | intra-seg PERFECT, cross-seg not | (won't fail — it's the known law; confirms it on the KV window) |
| 1c | hynix lawful coset; Micron 1 clean k=1 offset | no clean Micron offset ⇒ Micron forks fall back to bus read+write |
| 2a | drift CDF measured; onset recorded (D1/D3) | — (this arm produces the number) |

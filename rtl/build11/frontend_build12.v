        ...
        if(~((|delay_fin) || softmc_fin)) begin
            if(h2c_tvalid_0)
              ...
              if(maint_req && !program_pending) begin
                maint_ack = `HIGH;
                state_ns = EXECUTE_S;
                // build12: NO restart here — maintenance bypasses fetch
                // (its instructions come from maintenance_controller);
                // un-parking the previous user program's fetch mid-maint
                // was the prime suspect for the E14 silicon regression.
              end
            end
        end
      end

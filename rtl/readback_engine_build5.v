...
module readback_engine(
  ...
  // build5 = 0xDBC0DE03 (FWFT-safe trailer framing) so the host can
  // identify the image.
  // ------------------------------------------------------------------
  reg        flush_r;
...
`endif
  ...
  //  cnt_ref_init, cnt_zq_init, cnt_rd_init, 32'hDBC0DE03}
...
`endif
    ...
    if(switch_mode)
      mode_ns = ~mode_r;
    // build4: idempotent SET words (new frontend control words). They
    // win over the legacy toggle if ever co-asserted.
    if(set_mode_read)
      mode_ns = READ_MODE;
    if(set_mode_diff)
      mode_ns = DIFF_MODE;
...
`endif
  ...
  // (build5 = 0xDBC0DE03). build4: in accum mode the trailer also
...
`endif
  ...
  assign c2h_tdata_0  = trailer_beat ?
      {cnt_accum_write, cnt_drain, cnt_flush_eaten, cnt_flush_edge,
       cnt_ref_init, cnt_zq_init, cnt_rd_init, 32'hDBC0DE03} : rbf_dout;

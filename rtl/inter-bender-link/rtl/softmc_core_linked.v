...
// Per-DIMM DRAM-Bender core. Identical to softmc_top except that the XDMA
// endpoint and axis_clock_converters have been hoisted to the parent wrapper
// so one PCIe endpoint can drive multiple independent DDR4 controllers.
//
// softmc_core_linked = COPY of softmc_core.v with TWO surgical additions for
// the #76 inter-bender link (L25):
//   + 2 output ports (link_cfg_stb / link_cfg_data) exporting the route-config
//     control word decoded by frontend_linked (see frontend_linked.v).
//   + the `frontend` instance is `frontend_linked`, wired to those ports.
// Nothing else changes; with no LINK-CFG word sent it is bit-identical to core.
module softmc_core_linked #(parameter tCK = 1500, SIM = "false")
  (
  // common signals
  input c0_sys_clk_p,
  input c0_sys_clk_n,
  ...
  // MIG-generated clocks/resets exposed to parent so it can clock its own
  // axis_clock_converter that bridges back to the XDMA axi_clk domain.
  // (Renamed from the internal ddr4_ui_clk wire to avoid ANSI-port clash.)
  output                                core_ui_clk,
  output                                core_ui_rst,
  output                                core_init_calib_complete,

  // AXI-stream H2C (parent side runs on axi_clk; converter moves into ui_clk)
  input  [`XDMA_AXI_DATA_WIDTH-1:0]     m_axis_h2c_tdata_0,
  input                                 m_axis_h2c_tlast_0,
  input                                 m_axis_h2c_tvalid_0,
  output                                m_axis_h2c_tready_0,
  input  [`XDMA_AXI_DATA_WIDTH/8-1:0]   m_axis_h2c_tkeep_0,

  // AXI-stream C2H (ui_clk → axi_clk conversion handled in parent)
  output [`XDMA_AXI_DATA_WIDTH-1:0]     s_axis_c2h_tdata_0,
  output                                s_axis_c2h_tlast_0,
  output                                s_axis_c2h_tvalid_0,
  input                                 s_axis_c2h_tready_0,
  output [`XDMA_AXI_DATA_WIDTH/8-1:0]   s_axis_c2h_tkeep_0,

  // ---- NEW (#76 link): route-config export from frontend_linked ----
  output                                link_cfg_stb,   // 1-cycle pulse, this core's ui_clk
  output [15:0]                         link_cfg_data   // {magic[10:3],dst[2:1],en[0]}
  );

  // Exports happen at end-of-module (after the internal wires exist).
  ...
  // AXI-stream ports are now module I/O (see port list); no local wire decls.
  ...
  frontend_linked #(.SIM_MEM(SIM)) frontend(
    .clk(c0_ddr4_clk),
    .rst(c0_ddr4_rst),
    // ---- NEW (#76): route-config export ----
    .link_cfg_stb(link_cfg_stb),
    .link_cfg_data(link_cfg_data),
  ...
  // NOTE: the XDMA endpoint and AXI-stream clock converters live in the
  // parent wrapper (bcu1525_quad_top.v). The parent's converter presents
  // m_axis_h2c_* / s_axis_c2h_* already on this core's c0_ddr4_clk domain.
  ...
  // Export MIG clock/reset/cal-done for the parent's clock converters.
  assign core_ui_clk               = c0_ddr4_clk;
  assign core_ui_rst               = c0_ddr4_rst;
  assign core_init_calib_complete  = c0_init_calib_complete_r;
endmodule

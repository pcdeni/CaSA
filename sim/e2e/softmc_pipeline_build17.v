...
module softmc_pipeline(
  ...
  input                          ready_out,
  // build11: program-start strobe from the frontend -> fetch pc=0
  input                          restart,
  ...
  // build17: origin of the read commands leaving on ddr_read
  input                         data_out_maint,
  output                        ddr_read_maint
  );
  ...
  wire instr_maint_w, ddr_maint_w;
  fetch_stage fs(
    .data_in_maint(data_out_maint),
    .instr_maint(instr_maint_w),
    .restart(restart),
    .dbg_fetch_state(dbg_fetch_state),
    .dbg_fetch_pc(dbg_fetch_pc),
    .dbg_br_outstanding(dbg_br_outstanding),
    .clk(clk),
    .rst(rst),

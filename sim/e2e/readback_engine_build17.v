...
module readback_engine(
  ...
  input         set_mode_accxbp,   // build8: idempotent SET ACCUM_XBP mode
  input         set_acc_weight,    // build8: latch the per-plane weight
  input [3:0]   acc_weight_pl,     // build8: payload {neg, shift[2:0]}
  input         flush_acc,         // build8: arm the accumulator drain
  
  // DRAM <-> engine if
  input [511:0] rd_data,
  input         rd_valid,
  
  // build16: origin of the current flush (from the frontend, delayed
  // with it). Replaces the ignore_flush_ctr heuristic for deciding
  // whether a flush frames a USER record.
  input         flush_is_maint,
  // build16: HIGH while maintenance microcode is executing. Its read
  // announcements and its flush must not touch the user record's
  // accounting (maintenance runs thousands of times per streamed
  // session, so mis-attribution destroys the expectation).
  input         in_maint,
  // build17: the command bus, with the origin that travelled WITH the
  // instruction. Issue order is return order, so a queue built here is
  // correct by construction — unlike announcements (fetch order) or a
  // level sampled at issue/return (wrong pipeline stage).
  input [3:0]   ddr_read_issue,
  input         ddr_read_maint,
  // build16: acknowledge to the frontend. Pulses when a flush has been
  // RESOLVED — trailer transferred (tlast implies tvalid && tready) or
  // eaten. The frontend gates its bank swap on this instead of on a
  // fixed 32-cycle delay.
  output        flush_done,
  input         per_rd_init,
  input         per_zq_init,
  input         per_ref_init,
  ...
  output [1:0]  dbg_rbe_mode,
  output [15:0] dbg_rd_outstanding,
  output        dbg_ignore_read,
  output        dbg_proc_flush
  );

  // build15-dbg assigns live just above the endmodule.
  
  
  localparam READ_MODE   = 2'd0;
  localparam DIFF_MODE   = 2'd1;
  localparam SEG_POP_MODE = 2'd2;   // build7: per-32b-segment popcount readout
  localparam ACCUM_XBP_MODE = 2'd3; // build8: cross-bit-plane accumulator
  reg [1:0] mode_r, mode_ns; // READ / DIFF-accum / SEG_POP / ACCUM_XBP
...
`ifdef POPCOUNT_ACCUM_MODE
  ...
  // build16: a flush frames a user record unless the frontend says the
  // finishing program was maintenance. The armed-DIFF term is kept: an
  // accumulated drain is always delivered (build3 safety net).
  wire flush_proc = flush_edge && (~flush_is_maint
                                   || (mode_r == DIFF_MODE && accum_armed_r));
...
`endif
  ...
  reg[511:0] read_diff;
  reg        diff_valid;
  // build10: SEG_POP and ACCUM_XBP count RAW row data — their XOR
  // reference must be zero. ddr_wdata mirrors the core's wide_reg
  // (ddr_pipeline: ddr_wdata = ddr_data_r <= wide_reg), which PERSISTS
  // across programs; only the legacy INIT_MEM pass ever refreshed it.
  // Under build-9 streaming (EXECUTE->EXECUTE swaps, no INIT_MEM) a
  // write program's last LDWD pattern therefore XOR-polluted every
  // later count — sticky, silent, 3/4 of packed bytes wrong (silicon
  // 2026-07-22, stream-hw arms C-G). DIFF keeps its
  // compare-against-pattern XOR semantic. When ddr_wdata is 0 this
  // mask is an identity, so every legacy-validated path is unchanged.
  wire [511:0] diff_ref = (mode_r == DIFF_MODE) ? ddr_wdata : 512'b0;
  always @(posedge clk) begin
    if(rst) begin
      read_diff <= 512'bX;
      diff_valid <= `LOW;
    end
    read_diff <= rd_valid ? rd_data ^ diff_ref : read_diff;
  ...
  // acc word w (of 128) holds 16 signed 32-bit lanes; on every consumed
  // ACCUM_XBP beat, lane l of word w accumulates weight * popcount(segment
  // 16w+l), where the weight (sign + power-of-two shift, i.e. the
  // bitplane place-value factor) was latched from the SET_ACC_WEIGHT
  // control word between programs. FLUSH_ACC arms a drain: during the
  // NEXT program's flush window the FSM streams the 128 words into the
  // FIFO (same 256b half-swap as READ/SEG_POP) and zeroes them; the
  // trailer waits on ~acc_busy so framing stays one message. Beat
  // alignment is the proven seg_beat_valid/diff_valid pipeline; the word
  // index is the beat-within-row counter (full-row reads: 128 beats wrap
  // exactly). RMW is a 2-stage pipeline — consecutive beats hit
  // consecutive words, so no same-address hazard exists inside a row.
  // BRAM discipline: exactly one sync READ port (shared by the RMW
  // pipeline and the drain — never simultaneous by protocol: drain runs
  // only inside a no-read program's flush window) and one WRITE port
  // (RMW writeback / drain-zero, likewise exclusive).
  reg        acc_w_neg;
  reg [2:0]  acc_w_shift;
  reg [511:0] acc_mem [0:127];
  wire [6:0] acc_raddr;
  reg [511:0] acc_rdout;
  ...
  reg        acc_clearing;     // build8: zero the accumulator on mode entry
  reg [7:0]  acc_cidx;
  reg        acc_busy;
  reg        acc_dphase;       // 0: issue read of didx, 1: capture+push+zero
  reg [7:0]  acc_didx;
  reg [511:0] acc_dword;
  reg        acc_dword_valid;
  integer axi;
  assign acc_raddr = (rd_valid && ~ignore_read_r
                      && (mode_r == ACCUM_XBP_MODE))
                     ? axb_widx : acc_didx[6:0];
  always @(posedge clk) acc_rdout <= acc_mem[acc_raddr];
  always @* begin
    for (axi = 0; axi < 16; axi = axi + 1)
      axb_wr_word[axi*32 +: 32] = $signed(acc_rdout[axi*32 +: 32])
          + (acc_w_neg
              ? -$signed({26'b0, pc_out_l4[axi]} << acc_w_shift)
              :  $signed({26'b0, pc_out_l4[axi]} << acc_w_shift));
  end
  always @(posedge clk) begin
    if (rst) begin
      ...
      // build8: entering ACCUM_XBP zeroes the 128 accumulator words over
      // 128 cycles (acc_mem has no reset). The mode-set is followed by an
      // idle >128 cycles before any read, so the clear always completes
      // first; beats and drains are mutually exclusive with it.
      if (set_mode_accxbp) begin acc_clearing <= `HIGH; acc_cidx <= 8'd0; end
      else if (acc_clearing) begin
        acc_cidx <= acc_cidx + 8'd1;
        if (acc_cidx == 8'd127) acc_clearing <= `LOW;
      end
      if (set_acc_weight) begin
        acc_w_neg   <= acc_weight_pl[3];
        acc_w_shift <= acc_weight_pl[2:0];
      end
      if (flush_acc) acc_drain_pending <= `HIGH;
      // RMW: read issued at rd_valid (address axb_widx); one cycle later
      // pc_out_l4 (via read_diff) and acc_rdout are simultaneously valid
      // and the sum writes back. Consecutive beats touch consecutive
      ...
      if (read_seq_incoming && !rseq_r_axb
        ...
        axb_widx <= (axb_widx == 7'd127) ? 7'd0 : axb_widx + 7'd1;
      end
      // drain FSM. dphase 0 issues the read of didx (acc_raddr mux falls
      // through to acc_didx when no beat is in flight); dphase 1 captures
      // rdout, pushes it, and zeroes the word via the write port.
      acc_dword_valid <= `LOW;
      if (acc_drain_pending && !acc_busy && (mode_r == ACCUM_XBP_MODE)) begin
        acc_busy <= `HIGH; acc_drain_pending <= `LOW;
        acc_didx <= 8'd0; acc_dphase <= `LOW;
      end

      else if (acc_busy) begin
        if (acc_didx > 8'd127) begin
          acc_busy <= `LOW;
        end else if (!acc_dphase) begin
          acc_dphase <= `HIGH;             // read of didx issued this cycle
        end else if (!rbf_full) begin
          acc_dword <= acc_rdout;
          acc_dword_valid <= `HIGH;
          acc_didx <= acc_didx + 8'd1;
          acc_dphase <= `LOW;
        end
      end
      // single WRITE port: RMW writeback wins (never coincides with the
      // drain-zero by protocol; priority documents the intent).
      if (acc_clearing)
        acc_mem[acc_cidx[6:0]] <= 512'b0;
      else if (axb_beat_valid) begin
        acc_mem[axb_widx_d1] <= axb_wr_word;
      end
      else if (acc_busy && acc_dphase && !rbf_full && acc_didx <= 8'd127)
        acc_mem[acc_didx[6:0]] <= 512'b0;
    end
  end
  // ============== end build8 ACCUM_XBP ===================================
...
`endif
  ...
  rdback_fifo rbf(
    ...
    .din(mode_r == READ_MODE      ? {rd_data[255:0],rd_data[511:256]} :
         ...
         mode_r == ACCUM_XBP_MODE ? {acc_dword[255:0],acc_dword[511:256]} :
                                    {dsr_out[255:0],dsr_out[511:256]}),
    // build16: the trailer beat overrides c2h_tdata; popping the FIFO in
    // that cycle would discard a payload word. This coupling is why the
    // trailer previously had to wait for an empty FIFO.
    .rd_en(c2h_tready_0 && ~trailer_beat),
    .dout(rbf_dout),
    .valid(fifo_valid),
    .clk(clk),
    .srst(rst)
  ...
  // build16: pending-trailer counter. proc_flush_r == (pend != 0).
  // A single bit could not represent "two programs finished, one
  ...
  wire [15:0] n_issued = {15'd0, ddr_read_issue[0]} + {15'd0, ddr_read_issue[1]}
                       + {15'd0, ddr_read_issue[2]} + {15'd0, ddr_read_issue[3]};
  ...
  // recq       : expected payload BEATS for each record awaiting its
  //              trailer (queue, so overlap stays delimited)
  // rec_sent_r : payload beats transferred since the last trailer
  reg [15:0] rec_acc_r;
  reg [15:0] recq [0:7];
  reg [2:0]  recq_wr, recq_rd;
  reg [15:0] rec_sent_r;
  wire       recq_empty  = (recq_wr == recq_rd);
  wire       counted_mode = (mode_r == READ_MODE) || (mode_r == SEG_POP_MODE);
  // READ: one 512b read = 2 c2h beats. SEG_POP: 16 B per read = 1/2 beat.
  wire [15:0] rec_beats  = (mode_r == SEG_POP_MODE) ? (rec_acc_r >> 1)
                                                    : (rec_acc_r << 1);
  wire       rec_complete = ~recq_empty && (rec_sent_r >= recq[recq_rd]);
  wire       payload_xfer = c2h_tvalid_0 && c2h_tready_0 && ~trailer_beat;
  ...
  always @* begin
    ...
    flush_pend_ns = flush_pend_r;
    if(flush_edge) begin
      if(flush_proc) begin
        proc_flush_ns = `HIGH;
        // build16: remember EVERY owed trailer, not just "some".
        if(flush_pend_r != 4'hf) flush_pend_ns = flush_pend_r + 4'd1;
      end
      else
        flush_eaten = `HIGH;
    end
    // build8: an armed ACCUM_XBP drain opens the c2h message directly —
    // independent of the DIFF-mode flush accounting (accum_armed_r is
    // DIFF-only). The trailer-close gate additionally waits on
    // ~acc_busy && ~acc_drain_pending, so the 128 accumulator words are
    // all in the FIFO before tlast. FLUSH_ACC (set_acc → acc_drain_pending)
    // is the sole trigger; no flush_edge required.
    // proc_flush RISES exactly when the drain's first word enters the
    // FIFO (acc_dword_valid) — so the FWFT read never surfaces stale mem
    // (no empty-boundary prefix) — and the existing proc_flush_r hold
    // (proc_flush_ns = proc_flush_r above) keeps it high until tlast.
    if(mode_r == ACCUM_XBP_MODE && acc_dword_valid)
      proc_flush_ns = `HIGH;
...
`ifdef POPCOUNT_ACCUM_MODE
      ...
      // build16: an EMPTY FIFO IS NOT A RECORD BOUNDARY. Under c2h
      // backpressure the read credit throttles the DDR reads, so the
      // payload FIFO can drain to empty in the MIDDLE of a record and
      // the trailer drops there (observed: delimiters every 1216 B
      // instead of 2080). A record is complete only when every read it
      // announced has also returned -- rd_outstanding_r == 0.
      if(c2h_tready_0 && ~capture_pending_r && chunk_beats_r == 2'd0
         && (counted_mode ? rec_complete : (rbf_empty && ~dsr_valid))
         && ~acc_busy && ~acc_drain_pending) begin
`else
      if(c2h_tready_0 && rbf_empty && ~dsr_valid
         && ~acc_busy) begin
`endif
        tlast = `HIGH;
        // build16: one trailer just left; drop it from the owed count
        // (a same-cycle flush_edge increment above cancels correctly),
        // and stay armed while any trailer is still owed.
        if(flush_pend_ns != 4'd0) flush_pend_ns = flush_pend_ns - 4'd1;
        proc_flush_ns = (flush_pend_ns != 4'd0);
      end
      else
        proc_flush_ns = `HIGH;
    end
    else if(flush_pend_ns != 4'd0)
      proc_flush_ns = `HIGH;
    ...
    if(mode_r == SEG_POP_MODE && seg_beat_valid)
      buffer_space_ns = buffer_space_ns + 12'd2;
    // build8: ACCUM_XBP conservation — identical pattern (consumed beats
    // credit +2; the drain's c2h beats are auto-uncredited by the
    // ==READ_MODE gate on the transfer credit above).
    if(mode_r == ACCUM_XBP_MODE && axb_beat_valid)
      buffer_space_ns = buffer_space_ns + 12'd2;
  end
...
`endif
      ...
      // build17: one entry per ISSUING CYCLE. All reads issued in a
      // cycle come from one instruction, so they share an origin.
      if(|ddr_read_issue) begin
        rdq_cnt[rdq_wr]   <= n_issued;
        rdq_maint[rdq_wr] <= ddr_read_maint;
        rdq_wr            <= rdq_wr + 4'd1;
      end
      ...
      if(read_seq_incoming && ~flush_edge && ~in_maint)
        rec_acc_r <= rec_acc_r + {4'd0, incoming_reads};
      // any flush closes the accumulator; a PROCESSED flush also files
      // the expectation. An eaten (maintenance) flush just discards it,
      // so maintenance reads never leak into a user record.
      // only a USER flush closes the user accumulator; maintenance
      // flushes leave it untouched.
      if(flush_edge && ~flush_is_maint) begin
        rec_acc_r <= (read_seq_incoming && ~in_maint) ? {4'd0, incoming_reads}
                                                     : 16'd0;
        if(flush_proc) begin
          recq[recq_wr] <= rec_beats;
          recq_wr       <= recq_wr + 3'd1;
        end
      end
      // payload beats out; a delivered trailer consumes one expectation
      if(payload_xfer && ~(tlast && proc_flush_r))
        rec_sent_r <= rec_sent_r + 16'd1;
      if(tlast && proc_flush_r && ~recq_empty) begin
        rec_sent_r <= (payload_xfer ? rec_sent_r + 16'd1 : rec_sent_r)
                      - recq[recq_rd];
        recq_rd    <= recq_rd + 3'd1;
      end
    end
  end
...
`ifdef POPCOUNT_ACCUM_MODE
  ...
  // build16: in the streamed modes the boundary is the record's own
  // payload count; the FIFO may legitimately still hold the NEXT
  // record. Other modes keep the quiescent-FIFO rule.
  wire trailer_beat = proc_flush_r && ~capture_pending_r
                      && chunk_beats_r == 2'd0
                      && (counted_mode ? rec_complete
                                       : (rbf_empty && ~dsr_valid));
`else
  wire trailer_beat = proc_flush_r && rbf_empty && ~dsr_valid;
`endif

  // build16: one ack per resolved flush.
  // build16: the ack means "a USER record was closed on the wire".
  // Eaten maintenance flushes must NOT ack: maintenance runs constantly,
  // so acking them would clear the frontend's wait almost immediately
  // and re-open the overlap this handshake exists to prevent.
  assign flush_done   = tlast;
  ...
  assign c2h_tdata_0  = trailer_beat ?
      {cnt_accum_write, cnt_drain, cnt_flush_eaten, cnt_flush_edge,
       cnt_ref_init, cnt_zq_init, cnt_rd_init, 32'hDBC0DE0E} : rbf_dout;

  assign buffer_space = buffer_space_r >> 1;
  
  // build15-dbg: observation only
  assign dbg_rbe_mode        = mode_r;
  assign dbg_rd_outstanding  = rd_outstanding_r;
  assign dbg_ignore_read     = ignore_read_r;
  assign dbg_proc_flush      = proc_flush_r;

`ifdef SIM_TRAILER_DEBUG
  reg [31:0] dbg_stuck = 32'd0;
  reg [31:0] dbg_cyc = 32'd0;
  reg        dbg_flush_d = 1'b0;
  reg [31:0] dbg_n_flush_hi = 32'd0, dbg_n_edge = 32'd0, dbg_n_proc = 32'd0, dbg_n_tlast = 32'd0;
  reg [31:0] dbg_n_tl_valid = 32'd0, dbg_n_tl_xfer = 32'd0;
  always @(posedge clk) begin
    dbg_cyc <= dbg_cyc + 1;
    dbg_flush_d <= flush;
    if (flush) dbg_n_flush_hi <= dbg_n_flush_hi + 1;
    if (flush && !dbg_flush_d) dbg_n_edge <= dbg_n_edge + 1;
    if (flush_proc) dbg_n_proc <= dbg_n_proc + 1;
    if (tlast) dbg_n_tlast <= dbg_n_tlast + 1;
    if (tlast && c2h_tvalid_0) dbg_n_tl_valid <= dbg_n_tl_valid + 1;
    if (tlast && c2h_tvalid_0 && c2h_tready_0) dbg_n_tl_xfer <= dbg_n_tl_xfer + 1;
    if (tlast && !c2h_tvalid_0)
      $display("[rbe] TLAST WITHOUT TVALID cyc=%0d rbf_empty=%b dsr=%b trailer_beat_conds",
               dbg_cyc, rbf_empty, dsr_valid);
    if (dbg_cyc[17:0] == 18'h3FFFF)
      $display("[rbe] CENSUS cyc=%0d edges=%0d processed=%0d tlast_asserted=%0d tlast_valid=%0d tlast_XFER=%0d",
               dbg_cyc, dbg_n_edge, dbg_n_proc, dbg_n_tlast, dbg_n_tl_valid, dbg_n_tl_xfer);
  end
  // sim-only visibility into the framing decision (build16 bring-up)
  always @(posedge clk) begin
    if (flush_edge)
      $display("[rbe] t=%0t FLUSH_EDGE proc=%b eaten=%b ign_ctr=%0d pend=%0d rbf_empty=%b",
               $time, flush_proc, flush_eaten, ignore_flush_ctr_r, flush_pend_r, rbf_empty);
    if (tlast)
      $display("[rbe] t=%0t TLAST pend=%0d", $time, flush_pend_r);
    // stuck detector: proc_flush armed but no trailer leaving
    if (proc_flush_r) dbg_stuck <= dbg_stuck + 1; else dbg_stuck <= 0;
    if (dbg_stuck == 32'd20000)
      $display("[rbe] STUCK proc_flush: rbf_empty=%b dsr_valid=%b cap_pend=%b chunk=%0d acc_busy=%b acc_drain=%b tready=%b pend=%0d",
               rbf_empty, dsr_valid, capture_pending_r, chunk_beats_r,
               acc_busy, acc_drain_pending, c2h_tready_0, flush_pend_r);
  end
`endif

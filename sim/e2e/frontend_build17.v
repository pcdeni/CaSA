...
module frontend#(parameter SIM_MEM = "false")(
  ...
  // build16: origin of the flush that frontend_ready announces.
  // HIGH  -> the finishing program was maintenance microcode
  // LOW   -> it was a user program and MUST get its trailer
  output                        frontend_ready_maint,
  // build17: origin of the instruction word being served this cycle.
  // The frontend is where the choice between program and maintenance
  // microcode is made, so it is the only place the origin is known.
  output                        data_out_maint,
  // build16: readback acknowledge — the swap waits for the previous
  // record to be closed on the wire, not for a fixed delay.
  input                         flush_done,
  ...
  reg[31:0] delay_fin;
  reg[31:0] delay_fin_maint;   // build16: origin, shifted in lockstep
  // build16: maint_process one cycle back. Sampling it combinationally
  // AT the fin races the maint_ack that raises it in the same cycle,
  // which mis-tags a finishing USER program as maintenance and costs
  // that record its trailer. The delayed copy reflects the state during
  // the program's final instruction, which is unambiguous.
  reg maint_process_d1;
  // build16: set when a flush is issued (frontend_ready), cleared by the
  // readback engine's ack. While HIGH, program N's record is still open
  // on the wire and N+1 must not start.
  reg await_flush_ack;

  always @(posedge clk) begin
    if(rst || user_rst) begin
      delay_fin <= 32'b0;
      delay_fin_maint <= 32'b0;
      maint_process_d1 <= 1'b0;
      await_flush_ack <= 1'b0;
    end
    else begin
      delay_fin[1+:31] <= delay_fin[0+:31];
      delay_fin[0]    <= softmc_fin;
      delay_fin_maint[1+:31] <= delay_fin_maint[0+:31];
      // sample the origin AT the fin, so it travels with its own pulse
      delay_fin_maint[0]     <= softmc_fin & maint_process_d1;
      maint_process_d1       <= maint_process;
      // arm on the flush we just issued; disarm on the engine's ack.
      // (ack in the same cycle as the flush still clears it — a record
      // that closes immediately needs no wait.)
      // arm ONLY for user programs (maintenance never produces a
      // trailer, so waiting on one would deadlock the swap).
      if(delay_fin[31] && ~delay_fin_maint[31]) await_flush_ack <= 1'b1;
      if(flush_done)                            await_flush_ack <= 1'b0;
    end
  end

  // HIGH during the cycle a flush is issued: the ack cannot have been
  // observed yet, so never swap in that cycle.
  wire flush_done_pending = delay_fin[31];

  assign frontend_ready       = delay_fin[31];
  assign frontend_ready_maint = delay_fin_maint[31];
  ...
  assign data_out_maint  = ~program_process;
  ...
  always @* begin
    ...
    case (state_r)
        ...
        if(~((|delay_fin) || softmc_fin)) begin
            if(h2c_tvalid_0)
              ...
              if(maint_req && !program_pending) begin
                maint_ack = `HIGH;
                state_ns = EXECUTE_S;
                // build13: the restart pulse is BACK. Maintenance does
                // NOT bypass fetch — the microcode is muxed onto fetch's
                // IMEM-return path (program_process mux below) and fetch
                // is its conveyor. With park-at-END (build12), a parked
                // fetch drops maint words: maint_process sticks, the
                // h2c path jams (silicon 2026-07-24, e2e_sim repro).
                // Park semantics stay: park until the NEXT source, and
                // sources are {legacy load, stream swap, MAINT ENTRY}.
                fetch_restart = 1'b1;
              end
            end
        end
      ...
      EXECUTE_S: begin
        ...
        // build16: swap when the previous record is CLOSED, not merely
        // 32 cycles after its fin. frontend_ready arms the wait; the
        // engine's ack releases it.
        if(swap_pending_r && ~await_flush_ack && ~flush_done_pending) begin
